...
function rt(n, s, t, r = 0.3) {
  for (const a of s) {
    const o = a.a ?? 1, c = Math.min(1, Math.max(0, a.white)), M = t ? 1 - c : c;
    const h = Math.round(218 + 37 * M), m = Math.round(62 + 66 * M), D = Math.round(30 + 34 * M);
    n.fillStyle = `rgba(${h},${m},${D},${o})`, n.beginPath(), n.arc(a.x, a.y, a.r, 0, Math.PI * 2), n.fill();
  }
}
function it(n, s, t) {
  for (const r of s) {
    const a = r.a ?? 1, o = Math.min(1, Math.max(0, r.white)), c = t ? 1 - o : o;
    const M = Math.round(218 + 37 * c), h = Math.round(62 + 66 * c), m = Math.round(30 + 34 * c);
    n.strokeStyle = `rgba(${M},${h},${m},${a})`, n.lineWidth = r.w, n.beginPath(), n.moveTo(r.x1, r.y1), n.lineTo(r.x2, r.y2), n.stroke();
  }
}
