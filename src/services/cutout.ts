...
export class CutoutError extends Error {
  constructor(message: string, public readonly code = "cutout_error") {
    super(message);
...
export async function modalCutout(file: File): Promise<{ blob: Blob; metrics?: CutoutMetrics }> {
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new CutoutError(detail, `http_${response.status}`);
    }
    const blob = await response.blob();
    if (!blob.type.startsWith("image/") || blob.size === 0) throw new CutoutError("服务返回了无效图片，请重试。", "invalid_response");
    const metrics = parseMetrics(response.headers.get("X-Fangcun-Metrics"));
    console.info("[cutout] Modal result", { filename: file.name, inputBytes: file.size, outputBytes: blob.size, requestMs: Math.round(performance.now() - started), metrics });
    return { blob, metrics };
  } finally {
